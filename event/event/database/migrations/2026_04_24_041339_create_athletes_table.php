    ...
    public function up(): void
    {
        Schema::create('athletes', function (Blueprint $table) {
            ...
            $table->foreignId('perguruan_id')
                ...
                ->nullOnDelete() // Jika perguruan dihapus, atlet tidak ikut terhapus
                ->comment('Coach yang mendaftarkan atlet ini');
            $table->foreignId('disciplines_id')
                ->nullable()
                ->constrained('disciplines')
                ->nullOnDelete() // Jika disiplin dihapus, atlet tidak ikut terhapus
                ->comment('Disiplin yang diikuti atlet');
            $table->foreignId('age_category_id')
                ->nullable()
                ->constrained('age_categories')
                ->nullOnDelete()
                ->comment('Kategori usia atlet');
